...
export default function pageClient({ id }: { id: string }) {
    ...
    const getClient = async () => {
        try {
            const response = (await axios.get(`/api/Client?id=${id}`)).data
            setClient(response);
        } catch (error) {
            console.log(error)
        }
    ...
    const getPagamentosClient = async () => {
        try {
            const response = (await axios.get(`/api/Pagamentos?idClient=${id}`)).data;
            ...
            <div className="mt-8 w-full max-w-lg">
                ...
                <div className="bg-gray-800 rounded-lg p-4 shadow-md space-y-4">
                    {Array.isArray(pagamentos) && pagamentos.length > 0 ? (
                            ...
                            >
                                <div className="text-sm">
                                    <p className="font-medium text-gray-100">
                                        Data: {format(new Date(ele.data), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}
                                    </p>
                        ...
                        <p className="text-center text-gray-400">Nenhum pagamento encontrado.</p>
                    )}
